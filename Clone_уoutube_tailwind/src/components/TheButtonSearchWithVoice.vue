<template>
  ...
  <div :class="buttonHintClasses">
    Коснитесь микрофона, чтобы повторить попытку
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      buttonHintClasses () {
        return [
          this.isStatus(STATUS_LISTENING, STATUS_RECORDING) ? 'invisible' : 
          'visible', 'text-center', 'text-sm', 'text-gray-500', 'mt-4'
        ]
      },

      buttonAnimationClasses () {
        return [
          this.isStatus(STATUS_RECORDING) ? 'bg-gray-300' : 
          'border border-gray-300', 'animate-ping', 'absolute', 'w-14', 'h-14', 'rounded-full'
        ]
      }
    },
    ...
    watch: {
      status: {
        handler () {
          let text = 'Микрофон выключен. Попробуйте еще раз.'

          if (this.isStatus(STATUS_QUIET)) {
            text = "Не слышу. Попробуйте еще раз."
    ...
    // это функция жизненного цикла, которая вызывается перед тем, как компонент будет удален из DOM
    beforeUnmount () {
