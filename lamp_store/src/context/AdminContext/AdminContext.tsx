...
export const AdminContextProvider: FC<IChildProps> = ({ children }) => {
    ...
    const updateProduct = async (productData: IProduct) => {
        ...
        // объедяем старые данные с новыми. В противном случае возвращается старая лампа
        const updateLamp = lamps.map((lamp) => (lamp._id === productData.id ? { ...lamp, ...productData } : lamp));
        setLamps(updateLamp);
    };
