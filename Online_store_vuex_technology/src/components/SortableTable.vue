      ...
      <template v-if="products.length">
        <tr v-for="product of products" :key="product.id" class="d-flex">
          <template v-for="column of config" :key="column.id">
            <!-- - column.template - это метод или функция, которая принимает значение из объекта product по ключу column.id 
              и возвращает HTML содержимое -->
...
<script>
  export default {
    name: "SortableTable",
    props: ['data', 'tableConfig'], //массив товаров и конфигурация таблицы
    data() {
      return {
        products: this.data, //данные товаров
        config: this.tableConfig //конфигурация таблицы
      }
    },
    // отслеживается изменение свойства data, чтобы обновлять данные товаров при изменении внешних данных
    watch: {
      data (newValue) {
