			...
			<table class="table table-bordered table-hover">
				...
				<tbody>
					<tr v-for="product in products" :key="product.id">
						<td><router-link class="text-decoration-none" 
							:to="{ name: 'product', params: { id: product.id } }">{{ product.title.trim() }}</router-link></td>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// Метод dispatch во Vuex используется для вызова actions, При вызове передается имя 
			// нужного action и при необходимости данные, которые будут переданы в этот action
			placeOrder() {
				this.products.forEach(product => this.remove(product.id))
				this.$store.dispatch('cart/placeOrder', this.products)
			}
