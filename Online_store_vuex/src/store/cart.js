...
export default {
	...
	state: {
		items: [],
		token: null,
		orderPlaced: false, //таблица в заказах скрыта
		orderDetails: [], //массив значений в таблице заказов
	},
	getters: {
		...
		// Общее кол-во всех товаров в корзине
		length: (state, getters) => getters.itemsDetailed.reduce((total, item) => total + item.cnt, 0),
		// Общая стоимость всех товаров в корзине
		total: (state, getters) => getters.itemsDetailed.reduce((total, item) => total + item.price * item.cnt, 0)
	},
	mutations: {
		...
		// Код устанавливает значение переменной orderPlaced в состоянии хранилища Vuex равным переданному значению
		updateOrderPlaced(state, value) {
			state.orderPlaced = value
		},
		setItems(state, items) {
			state.items = items; // устанавливаем новый список товаров в корзине
		}
	},
	...
	actions: {
		...
		placeOrder({ commit }, products) {
			commit('setOrderDetails', { items: products })
			commit('updateOrderPlaced', true)
		},
		clearCart({ commit }) {
			commit('setItems', []) // очищаем список товаров в корзине
		}
	}
}
