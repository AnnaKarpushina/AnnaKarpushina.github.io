<template>
  <div class="absolute top-full w-full bg-white border border-t-0 border-gray-300 shadow-md pt-4">
    <ul>
      <li
    ...
    <a href="#" class="w-full inline-block text-right text-xs italic text-gray-500 hover:text-black pr-2" 
    @click="openSearchPredictionsModal">Сообщить о прогнозах поиска</a>
    <teleport to="body">
  ...
  export default {
    ...
    methods: {
      getItemClasses (resultId) {
        return [
          resultId === this.activeResultId ? 'bg-gray-100' : 'bg-transparent', 'text-black', 'px-3', 'py-1', 'select-none', 'truncate'
        ]
      },
