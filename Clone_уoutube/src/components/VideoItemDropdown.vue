  ...
  <div class="relative -mt-1 ml-auto">
    ...
    >
      ...
      >
        <section class="py-2">
          <ul>
            <VideoItemDropdownListItem label="Добавить в плейлист" icon="menuAlt3" />
          </ul>
        </section>
      </div>
    </transition>
...
<script>
  ...
  export default {
    ...
    computed: {
      buttonClasses () {
        return [
          'p-1', 'text-gray-500', 'hover:text-gray-700', 'focus:outline-none', 'group-hover:opacity-100',
          this.isOpen ? 'opacity-100' : 'opacity-0'
        ]
      },

      dropdownClasses () {
        return [ 'z-30', 'absolute', 'bg-white', 'w-60', 'rounded', 'shadow', 'focus:outline-none', ...this.positionClasses ]
      }
    },
    ...
    methods: {
      ...
      getTopClass (event) {
        const clickCoordY = event.clientY //вертикальную координату клика
        const buttonHeight = event.currentTarget.offsetHeight //высота кнопки
        const dropdownHeight = this.$refs.dropdown.offsetHeight //высота выпадающего списка
      ...
      getRightClass (event) {
        const clickCoordX = event.clientX //горизонтальная координата клика
        const clickCoordY = event.clientY //вертикальная координата клика
        const buttonHeight = event.currentTarget.offsetHeight
        const dropdownWidth = this.$refs.dropdown.offsetWidth //ширина выпадающего списка
